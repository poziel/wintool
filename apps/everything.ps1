...
# irm https://raw.githubusercontent.com/poziel/wintool/refs/heads/master/apps/everything.ps1 | iex

# To create a shortcut for Everything by Voidtools, execute the following line in PowerShell:
# $shortcut = (New-Object -ComObject WScript.Shell).CreateShortcut((Join-Path -Path (Get-Location) -ChildPath "Voidtools - Everything.lnk")); $shortcut.TargetPath = "%windir%\System32\WindowsPowerShell\v1.0\powershell.exe"; $shortcut.Arguments = "-ExecutionPolicy Bypass -Command `"Start-Process powershell.exe -verb runas -ArgumentList 'irm https://raw.githubusercontent.com/poziel/wintool/refs/heads/master/apps/everything.ps1 | iex'`""; $shortcut.WorkingDirectory = "%windir%\System32\WindowsPowerShell\v1.0"; $shortcut.Description = "Everything by Voidtools is a fast search utility to quickly locate files and folders on Windows."; $shortcut.Save()

# Get the download URL from the urlsearch script first
$Url = & ([scriptblock]::Create((Invoke-WebRequest -Uri "https://raw.githubusercontent.com/poziel/wintool/refs/heads/master/scripts/urlsearch.ps1").Content)) -url "https://www.voidtools.com/downloads/" -text "Download Portable Zip 64-bit"
